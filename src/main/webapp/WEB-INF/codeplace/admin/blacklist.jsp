...
<div>
    ...
    <table>
        ...
        <%
            List<User> sessionList = (List) session.getAttribute("blackList");
            pageContext.setAttribute("blackList", sessionList);
        %>

        <tbody>
        <c:forEach var="users" items="${blackList}">
            <tr>
    ...
    <c:if test="${pageMaker.isPrev()}">
        <a href="/adminBlackList?pageNum=${pageMaker.getStartPage()}">&laquo;</a>
    </c:if>
    <c:forEach var="pageNum" items="${pageMaker.getCountlist()}">
        &nbsp;<a href="/adminBlackList?pageNum=${pageNum}">${pageNum}</a>&nbsp;
    </c:forEach>
    <c:if test="${pageMaker.isNext()}">
        <a href="/adminBlackList?pageNum=${pageMaker.getEndPage()}">&raquo;</a>
    </c:if>
